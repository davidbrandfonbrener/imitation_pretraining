...
DATA_LIST = pretrain_data_configs(ABLATE_DATA_SIZE)
COMMON_DICT = dict(
...
# Hack to remove some datasets that we already trained on
DONE_LIST = ["point_mass", "metaworld_pick_place_nogoal", "kitchen_split_0"]
...
for data_config in DATA_LIST:
    for alg_config in ALG_LIST:
        ...
        if full_config["eval_env_name"] in DONE_LIST:
            continue
