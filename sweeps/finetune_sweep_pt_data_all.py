...
EXCLUDE_DATA = ["point_mass", "metaworld_pick_place_nogoal"]
DATA_LIST = [d for d in DATA_LIST if d["eval_env_name"] not in EXCLUDE_DATA]
print("Number of data configs:", len(DATA_LIST))
...
for data_config in DATA_LIST:
    for sweep_config in SWEEP_LIST:
        ...
        # Ensure we have 5 algs = 5 matches
        n_match = len(encoder_matches)
        assert n_match == 5, f"Found {n_match} matches for {full_config}"
